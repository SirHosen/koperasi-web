...
declare module '@/stores/modules/pinjamanVerifikasi' {
  ...
  export function usePinjamanVerifikasiStore(): {
    ...
    verifyDocument: (loanId: string, docId: string, status: 'diterima' | 'ditolak', notes?: string) => Promise<{ success: boolean }>;
    completeLoanVerification: (loanId: string, isApproved: boolean, notes?: string) => Promise<{ success: boolean }>;
    clearError: () => void;
  };
}