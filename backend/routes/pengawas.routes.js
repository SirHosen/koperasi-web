...
router.get('/compliance/monitoring', authenticateJWT, async (req, res) => {
  try {
    // Check if user is pengawas
    if (req.user.role !== 'pengawas') {
      return res.status(403).json({ error: 'Access denied. Pengawas role required.' })
...
// Get audit reports
router.get('/audit/reports', authenticateJWT, async (req, res) => {
  try {
...
router.post('/audit/reports', async (req, res) => {
  try {
    const { judul_audit, jenis_audit, tanggal_audit, temuan_utama, rekomendasi, auditor } = req.body
