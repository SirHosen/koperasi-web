...
router.post('/calculate', async (req, res) => {
  try {
    const { tahun, total_shu, persentase_jasa_modal, persentase_jasa_anggota, keterangan } =
      req.body
    ...
    try {
      ...
      await pool.execute('DELETE FROM shu WHERE tahun = ?', [tahun])
      ...
      for (const member of members) {
        ...
        const jasaAnggotaMember =
          totalKontribusi > 0
            ? (kontribusiMember / totalKontribusi) * jasaAnggota
            : jasaAnggota / members.length
