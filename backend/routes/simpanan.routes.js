...
router.get('/summary', authenticateJWT, async (req, res) => {
  try {
    ...
    const [anggotaRows] = await pool.execute(
...
router.put('/:id/verifikasi', [authenticateJWT, checkRole(['pengurus', 'admin'])], async (req, res) => {
  try {
    ...
    const [simpananRows] = await pool.execute(
    ...
    const newKeterangan = simpananRows[0].keterangan + 
    ...
    res.json({ 
      message: status === 'diverifikasi' 
        ? 'Simpanan berhasil diverifikasi' 
        : 'Simpanan ditolak'
    });
  } catch (error) {
